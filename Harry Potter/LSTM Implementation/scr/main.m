...
text = SynthesizeText(RNN_best, int_to_char, h_prev, c_prev, x_0, 200);
...
% Training loop
for ii = 1:epoch
    % Reset previous hidden state and c to zeros
    ...
    for e = 1:seq_length:(length(x_train) - seq_length - 1)
        ...
        if ~mod(itr, 10000) 
            text = SynthesizeText(RNN_train, int_to_char, h_prev, c_prev, x(:,1), 200);
            text = strcat(text, '\n');
            fprintf(text)
...
h_prev = zeros(RNN_best.m, 1);
c_prev = zeros(RNN_best.m, 1);
x_0 = ConvertOneHot('.', RNN_best.K, char_to_ind);

text = SynthesizeText(RNN_best, int_to_char, h_prev, c_prev, x_0, 1000);
fprintf(fileID, '\nFinal\n'); 
fprintf(fileID, text);
